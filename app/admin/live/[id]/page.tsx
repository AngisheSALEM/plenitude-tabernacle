...
import { Tv, ChevronLeft, ChevronRight, Play, Square, Settings, User } from "lucide-react"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <div className="flex items-center gap-2 text-muted-foreground mt-1">
            ...
            <span className="text-sm">Par: {sermon.author.firstName} {sermon.author.lastName}</span>
          </div>
        </div>
        <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/admin/sermons')}>
                Retour
            </Button>
            <Button
          ...
          <Card className="aspect-video flex items-center justify-center p-12 bg-black text-white relative group overflow-hidden border-4 border-primary/20">
            {currentSlide && (
                <div className="text-center space-y-4">
                    ...
                    {currentSlide.type === 'VERSE' && currentSlide.metadata && (
            ...
            {sermon.slides.map((slide: any, i: number) => (
                ...
                >
                    <CardContent className="p-3 text-xs line-clamp-3">
                        <div className="font-bold mb-1">{slide.type}</div>
                        {slide.content}
                    </CardContent>
                </Card>
            ))}
          ...
          <Card>
            <CardContent className="p-4 space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                    En tant qu'administrateur, vous contrôlez la diffusion des citations pour l'Espace Membre.
                </p>
                <div className="pt-4 border-t">
                    <div className="text-xs text-muted-foreground mb-1">ID Message:</div>
                    <div className="font-mono text-[10px] break-all bg-muted p-2 rounded">{sermon.id}</div>
                </div>
            </CardContent>
          </Card>
        </div>
